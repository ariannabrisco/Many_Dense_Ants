...
class AntGraph(nx.Graph):
    ...
    def add_node(self, node,  **attr):
        ...
        # we may want this to add a random unoccupied ant from the colony later, we may not, for now this function does
        # nothing
        # self.edges[node][Ant?]

    def add_edge(self, u_of_edge, v_of_edge, weight=1,  **attr):
        super().add_edge(u_of_edge, v_of_edge, **attr)
        # adds a base pheromone intensity to the path, this will later be incremented every time an ant completes his
        # trip across it
    ...
    # code from aco, it has yet to be converted, some or most of it may have to be completely replaced, I am not yet
    # sure, between colony and some of the additional things added by this just being a spicy networkx graph, a lot of
    # it will need to be reworked and/or can be done easier than it is currently being done, it's all currently
    # pretty messy
    ...
    def optimization(self, Q, alpha, beta, ants):  # ants is a list of ant instance
        ...
        for v in range(len(self.nodes)):
            for ant in ants:
                ...
                # probabilities of going to a neighbor
                greatest_prob = 0
                edge_to_take = None
                vertex_to_go = None
                potential_edges = []
                ...
                # edge_to_take.pher_amt = 0.5 * edge_to_take.pher_amt + Q / edge_to_take.weight
                # collect trail(pher amt) left by ant on that particular edge
                self.edges[edge_to_take]['pheromoneIntensity'] = (0.5 * self.edges[edge_to_take]['pheromoneIntensity'] +
                                                                  Q / self.edges[edge_to_take]['weight'])
                # collect trail(pher amt) left by ant on that particular edge
                # edge_to_take.prob = self.calculate_prob(edge_to_take, alpha, beta)
                # change the prob of that edge after the ant has left trail (pher amt)
                self.edges[edge_to_take]['prob'] = self.calculate_prob(edge_to_take, alpha, beta)  # change the prob of
                # that edge after the ant has left trail (pher amt)
